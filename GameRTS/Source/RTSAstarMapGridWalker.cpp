...
/************************************************************************************************************************/
  RTSAstarMapGridWalker::RTSAstarMapGridWalker( void )
  ...
  RTSAstarMapGridWalker::RTSAstarMapGridWalker( RTSTiledMap *pMap ) : RTSMapGridWalker( pMap )
  ...
  bool RTSAstarMapGridWalker::Init( sf::RenderTarget * target )
  ...
  void RTSAstarMapGridWalker::Destroy()
  {
  ...
  {//Función utilizada para renderear información del nodo en pantalla
    ...
    int32 tmpx;
    int32 tmpy;
    for ( uint32 i = 1; i < m_close.size() - 1; ++i )
    {
  ...
  void RTSAstarMapGridWalker::PathRender()
  {
    int32 tmpx;
    int32 tmpy;
  ...
  WALKSTATE::E RTSAstarMapGridWalker::Update()
  ...
  void RTSAstarMapGridWalker::visitGridNode( int32 x, int32 y )
  ...
  void RTSAstarMapGridWalker::PriorityQueue( int32 x, int32 y )
  {
    ...
    for ( auto it = m_open.begin(); it != m_open.end(); ++it )
    {
      ...
      if ( m_n->m_cost + distance < ( *it )->m_cost + distance2 )
      {

        m_open.insert( it, &m_nodegrid[x][y] );
        m_nodegrid[x][y].m_parent = m_n;
        m_nodegrid[x][y].m_cost += m_nodegrid[x][y].m_parent->m_cost;
        m_open.unique();
        return;
      }
    }
  ...
  void RTSAstarMapGridWalker::Reset()
  ...
  void RTSAstarMapGridWalker::traceBack()
  {
    if ( m_close.size() > 0 )
    {
